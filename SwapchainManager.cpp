	...
	framebufferHeight(frameBufferHeight)
{
	...
	renderpass = std::make_unique<RenderPass>(deviceManager.getDevice(), imageFormat, deviceManager.getSampleCount(),
	                                          depthBuffer->getDepthFormat());
	framebuffer = std::make_unique<Framebuffer>(deviceManager.getDevice(), imageViews, msaa->getImageView(),
...
void SwapchainManager::create()
{
	...
	uint32_t queueFamilyIndices[] = {
		deviceManager.getIndices().graphicsFamily.value(), deviceManager.getIndices().presentFamily.value()
	};
...
void SwapchainManager::recreate(uint32_t frameBufferWidth, uint32_t frameBufferHeight)
{
	...
	renderpass->create(deviceManager.getDevice(), deviceManager.getSampleCount(), depthBuffer->getDepthFormat(),
	                   imageFormat);
